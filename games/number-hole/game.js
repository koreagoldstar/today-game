(() => {
  ...
  const STAGES = Array.from({ length: TOTAL_STAGES }, (_, i) => ({
    ...
    goal: 10 + Math.floor(i * 1.5),
    startPower: Math.min(2 + Math.floor(i / 5), 5),
    rivals: Math.min(2 + Math.floor(i / 5), 6),
    food: Math.min(32 + i, 48),
    time: Math.max(55, 90 - i),
    aggress: Math.min(0.9, 0.48 + i * 0.014),
    bigChance: Math.min(0.38, 0.16 + i * 0.008),
  }));
  ...
  function updateHud() {
    ...
    const rivalsEl = document.getElementById("hud-rivals");
    if (rivalsEl) rivalsEl.textContent = String(rivals.length);
  ...
  function spawnAroundPlayer(minDist, maxDist) {
    const ang = rand(0, Math.PI * 2);
    const dist = rand(minDist, maxDist);
    return {
      x: clamp(player.x + Math.cos(ang) * dist, 100, WORLD - 100),
      y: clamp(player.y + Math.sin(ang) * dist, 100, WORLD - 100),
    };
  }

  function makeFood(forced, nearby = false) {
    const st = STAGES[stageIndex];
    const base = player ? player.power : st.startPower;
    const p = forced == null ? rollFoodPower(base, st) : forced;
    const pos = nearby && player
      ? spawnAroundPlayer(150, 980)
      : { x: rand(120, WORLD - 120), y: rand(120, WORLD - 120) };
    return {
      x: pos.x,
      y: pos.y,
      power: Math.max(1, p),
  ...
  function makeRival(forced, nearPlayer) {
    ...
    if (player && nearPlayer !== false) {
      const pos = spawnAroundPlayer(580, 1100);
      x = pos.x;
      y = pos.y;
    } else {
      do {
    ...
    return {
      ...
      mouth: 0, facing: 0, alive: true, rage: 0, stun: 0, pack: Math.random() < 0.3,
    };
  }
  ...
  function resetStage() {
    ...
    buildCity();
    for (let i = 0; i < st.food; i++) {
      // Most score items stay in the active region; some reward exploration.
      foods.push(makeFood(null, i < Math.ceil(st.food * 0.72)));
    }
    // Deliberate mix: prey + peer in tutorial stages, predators from stage 3.
    for (let i = 0; i < st.rivals; i++) {
      let power = Math.max(1, st.startPower - 1);
      if (i === 1) power = stageIndex < 2 ? st.startPower : st.startPower + 1;
      if (i >= 2) power = i % 2 === 0
        ? st.startPower
        : st.startPower + 1 + Math.floor(stageIndex / 10);
      rivals.push(makeRival(power, true));
    }
    hintTimer = 3;
    hint.classList.remove("fade");
    hint.textContent = "작은 라이벌은 삼키고, 큰 라이벌에게 잡히면 끝!";
    updateHud();
  }
  ...
  function canAbsorbRival(eaterPower, preyPower, dist) {
    const capture = (powerRadius(eaterPower) + powerRadius(preyPower)) * 0.62;
    return preyPower < eaterPower && dist < capture;
  }
  ...
  function resolvePlayerRival(r, i, dt) {
    ...
    // The smaller body is pulled into the larger core before capture.
    if (r.stun <= 0 && r.power < player.power && dist < sumR * 1.55) {
      const ang = Math.atan2(player.y - r.y, player.x - r.x);
      r.x += Math.cos(ang) * 125 * dt;
      r.y += Math.sin(ang) * 125 * dt;
      dist = Math.hypot(r.x - player.x, r.y - player.y);
    }

    if (canAbsorbRival(player.power, r.power, dist)) {
    ...
    if (player.invuln <= 0 && canAbsorbRival(r.power, player.power, dist)) {
      burst(player.x, player.y, "#ff6b5a", 20);
      endGame("eaten");
      return "dead";
    }

    // Equal powers collide. Unequal powers must overlap so absorption can happen.
    if (r.power === player.power && dist < sumR * 0.94) {
    ...
    } else if (player.invuln > 0 && r.power > player.power && dist < sumR * 0.88) {
      // Start protection should repel predators rather than leave them overlapping.
      separateBodies(player, r, sumR);
      knockRival(r, player.x, player.y, 200);
    }
    return "ok";
  }
  ...
  function update(dt) {
    ...
    for (let i = 0; i < rivals.length; i++) {
      for (let j = i + 1; j < rivals.length; j++) {
        ...
        if (canAbsorbRival(a.power, b.power, d)) {
          rivalGrow(a, 2);
          b.alive = false;
          burst(b.x, b.y, b.color, 8);
        } else if (canAbsorbRival(b.power, a.power, d)) {
          rivalGrow(b, 2);
          a.alive = false;
    ...
    if (spawnAcc > 0.65 && foods.length < st.food) {
      spawnAcc = 0;
      foods.push(makeFood(null, true));
    }
    rivalAcc += dt;
    if (rivalAcc > Math.max(8, 12 - stageIndex * 0.12) && rivals.length < st.rivals) {
      rivalAcc = 0;
      const roll = Math.random();
      const power = roll < 0.4
        ? Math.max(1, player.power - 1)
        : roll < 0.65
          ? player.power
          : stageIndex < 2
            ? player.power
            : Math.min(MAX_POWER, player.power + 1 + Math.floor(stageIndex / 10));
      rivals.push(makeRival(power, true));
    }
    updateHud();
