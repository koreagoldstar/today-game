(() => {
  ...
  const GAME_ID = "rhythm-easy";
  const GAME_TITLE = "리듬 톡톡 지혁이용";
  const ASSET_BASE = "/games/rhythm/assets/";
  ...
  const SONG_META = CYBER_NAMES.map((name, i) => {
    ...
    const rate = Math.min(1.02, 0.78 + (i % 6) * 0.03);
    const bpm = Math.round(Math.min(118, Math.max(82, TRACKS[track].baseBpm * rate)));
    const diff = Math.min(2, 1 + Math.floor(i / 25));
    return {
      name,
      bpm,
      style: "cyber",
  ...
  function buildChart(meta, index) {
    const rand = mulberry32(7000 + index * 97);
    const beat = 60 / meta.bpm;
    const bars = 7 + meta.diff + Math.floor(rand() * 2);
    const dens = 0.24 + meta.diff * 0.07;
    const pool = PATTERNS[Math.min(2, meta.diff)] || PATTERNS[1];
    const chart = [];
    let t = 1.2;
    for (let b = 0; b < bars * 4; b += 1) {
      if (rand() < dens) {
        ...
        for (const lane of lanes) {
          chart.push({ lane, time: +(t + (rand() - 0.5) * 0.015).toFixed(3) });
        }
      }
      t += beat;
    }
  ...
  function registerMiss(lane) {
    ...
    if (lives <= 0) {
      ...
      TodayGameRank.mount({ gameId: GAME_ID, gameTitle: GAME_TITLE, formParent: document.getElementById("over") });
  ...
  function checkSongEnd() {
    ...
    if (allDone && songTime >= songEnd - 1.5) {
      ...
      if (songIndex >= SONGS.length - 1) {
        ...
        showOverlay("all");
        if (window.TodayGameRank) {
      TodayGameRank.mount({ gameId: GAME_ID, gameTitle: GAME_TITLE, formParent: document.getElementById("allclear") });
      TodayGameRank.open(score);
    }
      } else {
